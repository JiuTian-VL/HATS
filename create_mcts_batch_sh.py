...
# List of package names to process, one per tmux window
PACKAGE_NAMES = [
...
# Initial device serial number (the numeric part)
INITIAL_DEVICE_SERIAL_NUM = 5554

# --- Script and Environment Settings ---
SCRIPT_DIR = os.path.dirname(os.path.abspath(__file__))
TARGET_MCTS_SCRIPT_NAME = "mcts_main.py"  # Make sure this is correct
CONDA_ENV_NAME = "mcts"  # Ensure this matches your environment
PYTHON_EXECUTABLE = "python"

# --- Default MCTS Parameters (used as fallback) ---
DEFAULT_MCTS_PARAMS = {
    "MAX_SIMULATION_STEPS": 6,
...
# Keys are package names, values are dictionaries of parameters to override.
# If a package is not listed, or a parameter is not specified for a listed package,
...
APP_SPECIFIC_MCTS_PARAMS = {
    ...
    "com.android.chrome": {
        "MAX_EXECUTION_STEPS": 22,
        "MAX_SIMULATION_STEPS": 22,
    },
    "com.google.android.documentsui": {
        "MAX_EXECUTION_STEPS": 20,
        "MAX_SIMULATION_STEPS": 20,
    ...
    "com.arduia.expense": {
        "MAX_EXECUTION_STEPS": 35,
        "MAX_SIMULATION_STEPS": 35,
    },
    "com.simplemobiletools.gallery.pro": {
        "MAX_EXECUTION_STEPS": 18,
        "MAX_SIMULATION_STEPS": 18,
    },
    "net.gsantner.markor": {
        "MAX_EXECUTION_STEPS": 35,
        "MAX_SIMULATION_STEPS": 35,
    ...
    "com.simplemobiletools.calendar.pro": {
        "MAX_EXECUTION_STEPS": 34,
        "MAX_SIMULATION_STEPS": 34,
    },
    "com.simplemobiletools.draw.pro": {
        "MAX_EXECUTION_STEPS": 18,
        "MAX_SIMULATION_STEPS": 18,
...
# --- Tmux Configuration ---
SESSION_NAME = "mcts"
OUTPUT_SCRIPT_PATH = os.path.join(SCRIPT_DIR, "run_mcts_batch.sh")
ROOT_DATA_DIR = "./mcts_output"  # Base directory for output
...
def get_mcts_params_for_app(package_name):
    ...
    overrides = APP_SPECIFIC_MCTS_PARAMS.get(package_name, {})
...
def create_batch_script_sendkeys_v2():
    ...
    script_content.append(f"# Number of jobs: {len(PACKAGE_NAMES)}")
    ...
    target_script_full_path = os.path.join(SCRIPT_DIR, TARGET_MCTS_SCRIPT_NAME)
    script_content.append(f"if [ ! -f {shlex.quote(target_script_full_path)} ]; then")
    script_content.append(
        f'    echo "Error: Target MCTS script not found at {target_script_full_path}"'
    )
    script_content.append("    exit 1")
    script_content.append("fi\n")
    ...
    script_content.append(
        f"\necho \"Configuring {len(PACKAGE_NAMES)} windows in tmux session '$SESSION_NAME' via send-keys...\"\n"
    )
    ...
    # Create commands for each package name
    for i, package_name in enumerate(PACKAGE_NAMES):
        current_serial_num = INITIAL_DEVICE_SERIAL_NUM + i * 2
        device_serial = f"emulator-{current_serial_num}"
        window_name = f"mcts_{i}_{package_name.split('.')[-1]}"  # More descriptive name
        ...
        mcts_params = get_mcts_params_for_app(package_name)
        ...
        python_run_command = (
            f"{PYTHON_EXECUTABLE} {shlex.quote(TARGET_MCTS_SCRIPT_NAME)} "
        ...
        script_content.append(
            f"# --- Window {i}: {package_name} on {device_serial} ---"
        )
        script_content.append(
    ...
    script_content.append(
        f"\n# Finished sending commands to all {len(PACKAGE_NAMES)} windows."
    )
    script_content.append(f"# Attach to the session to view progress:")
    script_content.append(f"# tmux attach-session -t $SESSION_NAME")
    script_content.append(
        f"echo \"\nTmux session '$SESSION_NAME' setup initiated with {len(PACKAGE_NAMES)} windows via send-keys.\""
    )
...
def main():
    ...
    try:
        ...
        print(
            f"  3. Ensure required Android emulators (e.g., emulator-{INITIAL_DEVICE_SERIAL_NUM}, emulator-{INITIAL_DEVICE_SERIAL_NUM+2}, ...) are running."
        )
        print(
            f"  4. Execute the generated script: ./{os.path.basename(OUTPUT_SCRIPT_PATH)}"
        )
        print(
            f"     (This script will create/configure the tmux session and then exit.)"
        )
        print(
            f"  5. Attach to the tmux session: tmux attach -t {shlex.quote(SESSION_NAME)}"
        )
        print(f"  6. Each window will run MCTS with potentially different parameters.")
        print(
            f"  7. Windows will remain open showing script output/shell prompt after completion."
        )
