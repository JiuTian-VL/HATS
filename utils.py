...
from prompt_templates import GENERATE_INPUT_CONTENT_PROMPT
...
from prompt_templates import TRAJECTORY_TO_INSTRUCTION_PROMPT



from device import _generate_ui_element_description
...
def trajectory_to_instruction(
    ...
    for i in range(len(trajectory_data["actions"])):
        ...
        package_name = trajectory_data["package_names"][i]
        ...
        trajectory_information += f"Package Name: {package_name}\n\n"
    ...
    response_text = openai_request(
        messages=messages, timeout=600, max_tokens=32768, usage=usage
    )
